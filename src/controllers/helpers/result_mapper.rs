...
pub fn map_domain_error(err: DomainError) -> HttpResponse {
    match err {
        // NOT FOUND
        DomainError::ShipmentNotFound { tracking_number } => HttpResponse::NotFound().json(json!({
            "error": "ShipmentNotFound",
            "message": format!("Shipment {} not found", tracking_number)
        })),

        DomainError::ShipmentNotFoundById { id } => HttpResponse::NotFound().json(json!({
            "error": "ShipmentNotFound",
            "message": format!("Shipment with id {} not found", id)
        })),
        DomainError::PaymentNotFound { reference } => HttpResponse::NotFound().json(json!({
            "error": "PaymentNotFound",
            "message": format!("Payment {} not found", reference)
        })),

        DomainError::PaymentWithShipmentIdNotFound { shipment_id } => HttpResponse::NotFound()
            .json(json!({
                "error": "PaymentNotFound",
                "message": format!("Payment with id: {} not found", shipment_id)
            })),
        DomainError::RevenueNotFoundWithDate { date } => HttpResponse::NotFound().json(json!({
            "error": "RevenueNotFound",
            "message": format!("Revenue for this day: {} not found", date)
        })),
        DomainError::RevenueNotFound { month } => HttpResponse::NotFound().json(json!({
            "error": "RevenueNotFound",
        ...
        DomainError::ShipmentNotDelivered => HttpResponse::BadRequest().json(json!({
            "error": "ShipmentNotDelivered",
            "message": "Shipment must be delivered before this operation"
        })),

        DomainError::ProofMustContainImageOrNote => HttpResponse::BadRequest().json(json!({
            "error": "ProofValidationError",
            "message": "Proof must contain either an image or a note"
        })),

        DomainError::SubmittedByEmptyError => HttpResponse::BadRequest().json(json!({
            "error": "SubmittedByEmpty",
            "message": "Submitted_by cannot be empty"
        })),

        DomainError::PaymentGatewayError { signature } => HttpResponse::BadRequest().json(json!({
            "error": "ShipmentNotDelivered",
            "message": format!("Invalid signature {} ", signature)
        })),
        // CONFLICT (BUSINESS RULES)
        DomainError::DuplicateProofOfDelivery => HttpResponse::Conflict().json(json!({
            "error": "DuplicateProof",
            "message": "Proof of delivery already exists"
        })),

        DomainError::DuplicateEntity => HttpResponse::Conflict().json(json!({
            "error": "DuplicateEntity",
            "message": "Entity already exists"
        })),
        DomainError::PaymentExistsForThisShipment { id } => HttpResponse::Conflict().json(json!({
            "error": "Duplicate",
        ...
        | DomainError::NumericOverflow => HttpResponse::BadRequest().json(json!({
        ...
        | DomainError::SerializationFailure => HttpResponse::ServiceUnavailable().json(json!({
            "error": "TransientDatabaseError",
            "message": "Temporary database issue, please retry"
        })),

        DomainError::DatabaseError(msg) => HttpResponse::InternalServerError().json(json!({
            "error": "DatabaseError",
            "message": msg
        })),
...
pub fn extract_or_bad_request<T>(res: Result<T, DomainError>) -> Result<T, HttpResponse> {

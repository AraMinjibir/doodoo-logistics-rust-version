...
use crate::domain::models::{payment::Payment, payment::PaymentMethod};
...
impl PaymentRow {
    ...
    pub fn into_domain(self) -> Payment {
        Payment::reconstitute(
            ...
            self.shipment_id,
            self.amount,
            PaymentStatus::from_string(&self.status).expect("Invalid payment status in DB"),
            self.paid_at,
            PaymentMethod::from_string(&self.payment_method).expect("Invalid payment method in DB"),

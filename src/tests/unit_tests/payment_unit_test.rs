...
use crate::domain::gateways::{payment_gateway::PaymentGatewayResponse, payment_gateway::PaymentWebhookEvent};
...
async fn generate_payment_sucess(){
    ...
    let callback_url = format!("https://mock-payments.local/pay/{}", payment.reference_number());

    let payment_result =
     service.generate_payment(callback_url, &payment.clone()).await;
...
async fn generate_paymnent_failure(){
    ...
    shipment_repo.expect_create()
    .returning(|_| Err(RepositoryError::DatabaseError("fail".into())));

    shipment_repo.expect_get_by_id()
    .returning(|_|Err(RepositoryError::DatabaseError("ShipmentNotFound".to_string())));
...
async fn get_payment_by_ref_not_found(){
    ...
    assert!(matches!(fetched_payment, Ok(None)));
}
...
async fn handle_webhook_success() {
    ...
    gateway.expect_verify_webhook()
    .returning(|_, _| {
        Ok(PaymentWebhookEvent {
            reference: "mock-ref".to_string(),
            status: "success".to_string(),
            gateway_transaction_id: Some("id".to_string()),
        })
    });
...
async fn handle_webhook_invalid_signature() {
    ...
    let result = service
        .handle_webhook("payload", "bad-signature")
        .await;
...
async fn handle_webhook_payment_not_found() {
    ...
    assert!(matches!(
        result,
        Err(DomainError::PaymentNotFound { .. })
    ));
}
